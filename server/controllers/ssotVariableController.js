...
const initiateParametersForActivity = async (
  robotId,
  activityId,
  updatedParameters,
  hasOutput
...
const checkForOutputValue = async (robotId, activityId) => {
  const parameterObject = await mongoose
    .model('parameter')
    .findOne({
      robotId,
      activityId,
...
const updateParametersForActivity = async (
  robotId,
  activityId,
  updatedParameters,
  updatedOutput,
...
exports.getVariablesForNewTask = async (req, res) => {
  try {
    ...
    const rpaTaskVariables = await findParametersForTask(
      applicationWithEmptyspace,
      taskWithEmptyspace
    );
...
exports.getVariables = async (req, res) => {
  try {
    ...
    const variables = await mongoose
      .model('parameter')
      .findOne({
        robotId: robotId,
      })
...
exports.updateOnlyInputParams = async (req, res) => {
  try {
    ...
    const updatedParameterObject = await mongoose
      .model('parameter')
      .findOneAndUpdate(
        {
          robotId,
