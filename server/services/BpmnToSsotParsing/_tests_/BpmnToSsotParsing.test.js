...
const BPMN_JSON = require('./BpmnMockForTesting.json');
const testSsot = require('./SsotMockForTesting.json');

const Ssot = JSON.parse(parser.parseBpmnToSsot(BPMN_JSON));

describe('Parsing Tests', () => {
  test('bpmn fully parsed to Ssot', () => {
    expect(JSON.stringify(Ssot, null, 2)).toMatch(
      JSON.stringify(testSsot, null, 2)
    );
  });
  ...
  test('Ssot contains flow', () => {
    Ssot.elements.forEach((element) => {
      expect(
        element.predecessorIds.length + element.successorIds.length
      ).toBeGreaterThan(0);
    });
  });
});
