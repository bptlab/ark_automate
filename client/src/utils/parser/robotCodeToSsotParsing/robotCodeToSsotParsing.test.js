...
import { correctSettingsSection, correctTaskSection, taskAndApplicationCombinations, correctElementsArray, correctInstructionBlocks } from './robotCodeTestSnippets';
...
const settingsSectionAsArray = parser.getRobotCodeAsArray(correctSettingsSection);
...
describe('RobotCode to SSOT Parsing Tests', () => {
    ...
    test('settings selector was found correctly', () => {
        ...
        expect(parser.getLineNumberForSelector(settingsSectionAsArray, '*** Settings ***')).toBe(0);
...
describe('Error handling while parsing', () => {
    test('settings selector wasn\'t found (and Error was thrown)', async () => {
        customNotification.mockImplementation((type, message) => {
            ...
            expect(message).toEqual('The required selector "*** Settings ***" was not found');
        });

        const robotCodeWithOutSettingsSelector = robotCodeAsArray.slice(1);
        parser.getLineNumberForSelector(robotCodeWithOutSettingsSelector, '*** Settings ***');
    });

    test('check "Library" error handling', async () => {
        customNotification.mockImplementation((type, message) => {
            expect(type).toEqual('Error');
            expect(message.includes(`Every line of the "*** Settings ***" Section has to start with "Library"! \nError location: "`)).toBe(true);
        });

        const settingsSectionWithError = settingsSectionAsArray.map((singleLine) => singleLine.replace("Library", "Lib"))
    ...
    test('check "RPA."-Alias error handling', async () => {
        customNotification.mockImplementation((type, message) => {
            expect(type).toEqual('Error');
            expect(message.includes('Application has to start with "RPA." \nError location:')).toBe(true);
        });

        const settingsSectionWithError = settingsSectionAsArray.map((singleLine) => singleLine.replace("RPA.", "Rpa."))
        parser.getApplicationArray(settingsSectionWithError);
    });
    ...
    test('check "RPA-Application is not defined" error handling', async () => {
        customNotification.mockImplementation((type, message) => {
            expect(type).toEqual('Error');
            expect(message.includes('The Application "Word.Application" is currently not supported.')).toBe(true);
        });

        const settingsSectionWithError = settingsSectionAsArray.map((singleLine) => singleLine.replace("Excel.Application", "Word.Application"))
        parser.getApplicationArray(settingsSectionWithError);
    });
});
