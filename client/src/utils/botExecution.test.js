import { checkForExecutability } from './botExecution';
...
describe('Checking Bot for Executability', () => {
  it('correctly assigns bot as executable', async () => {
    ...
    const response = checkForExecutability(correctListOfParameters);
    expect(response).toBe(true);
  });

  it('correctly assigns bot as not executable when required parameter is empty', async () => {
    const incorrectListOfParameters = [paramObject1, paramObjectWithoutRequiredValue];
    const response = checkForExecutability(incorrectListOfParameters);
