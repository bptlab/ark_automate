...
var bpmnTasks = json_data["definitions"]["foo"]["process"]["task"];
...
for (task in bpmnTasks) {
    let rpaApp = bpmnTasks[task].extensionElements.properties.property[0]._value;
...
for (task in bpmnTasks) {
    ...
    if (rpaTask == ("Set Worksheet Value")) {
        codeToAppend +="   Set Worksheet Value  "+rpaTaskProps[2]["_value"]+"  "+rpaTaskProps[3]["_value"]+"  "+rpaTaskProps[4]["_value"]+"\n"
